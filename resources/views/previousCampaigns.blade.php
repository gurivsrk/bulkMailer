                                        ...
                                        <tr class="{{$campaign->status == 'sending'? 'warning':''}}">
                                            ...
                                            <td class="text-center relative">
                                                ...
                                                <div class="w-full {{$campaign->status == 'fail'?'bg-red-200':'bg-gray-200'}} rounded-full dark:bg-gray-700">
                                                        <div class="{{$campaign->status == 'fail'?'bg-red-600':'bg-blue-600'}} text-xs font-medium  {{ $campaign->status!='completed' ?  ($campaign->status != 'fail' ? ($per < 30 ? 'text-blue-900': 'text-blue-100') : 'text-slate-800') :'text-blue-100' }} text-center p-0.5 leading-none rounded-full" style=" {{$campaign->status=='completed' && $campaign->status != 'fail'? 'width: 100%' : 'width:'.$per.'%'}}">{{$campaign->status=='completed' ? 'completed' : ($campaign->status == 'fail' ? 'failed' :$per.'%')}}</div>
                                                </div>
